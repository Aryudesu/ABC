...
def is_ok_dist(mid, pt, k, bm: bisectMapper):
    """ptに対して距離midの個数がk個未満か"""
    return bm.between_c_c(pt - mid, pt + mid)< k

def search_dist(b, k, bm):
    """
    左側に対しての判定で二分探索実行
    ===
...
result = []
for q in range(Q):
    b, k = [int(l) for l in input().split()]
    l, r = search_dist(b, k, bm)
    if bm.between_c_c(b - l, b + l) == k:
        print(l)
    else:
        result.append(r)
for r in result:
